    ...
    def __init__(self, expr="") -> None:
        ...
        while i < len(chars) - 1:
            if chars[i] == DYNAMIC_RANGE_SPECIFIER:
                ...
                del chars[i-1:i+2]
                i -= 2
                toExtend += self._getRange(prev, next)
            i += 1
...
class Statement:
    ...
    indicating how many times it will be repeated and
    how (if at all). The value can be a StaticString,
    a DynamicString, a Variable, or a Join.
    """
    ...
    def size(self):
        #print([self.value.size()**(self.quantifier.begin() + i) for i in range(self.quantifier.size())])
        return sum([self.value.size()**(self.quantifier.begin() + i) for i in range(self.quantifier.size())]) 
...
class StaticString:
    ...
    def generate(self):
        yield "".join(c.value for c in self.chars)
